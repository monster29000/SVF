...
static llvm::cl::opt<bool> NPDVFG("npdvfg", llvm::cl::init(false),
                                      llvm::cl::desc("use vfg for NULL-Pointer-Dereference Detection"));
...
void SrcSnkDDA::initialize(SVFModule* module)
{
    ...
    if(NPDVFG)
        ...
        svfg = memSSA.buildPTROnlySVFG((BVDataPTAImpl*)pta);
...
void SrcSnkDDA::FWProcessOutgoingEdge(const DPIm& item, SVFGEdge* edge)
{
    ...
    {
        SVFUtil::outs() << "GlobalSVFGNode : " << dstNode->toString() << "\n";
